...
These six checks explain the scope of [the covariance theorem](THEOREM.md), including the distinction between fixed boundary access and operations on the whole growing halves.
...
**Fixed boundary SWAP.** If $`r=s=q`$ is fixed and $`U`$ swaps just these boundary factors of dimension $`q`$, $`\mathcal R(U)`$ is a $`q^2\times q^2`$ permutation matrix. Thus $`\eta`$ has $`q^2`$ equal entries $`1/q^2`$, and
