...
One active paper: **Operator Schmidt spectra and entanglement covariance under local gates**. Target: Quantum. Working project name: Gate Entanglement Covariance. Current authorized repository: GoGoKo699/Entangling-successions, pending any later rename.
...
Run `python verify_project.py` for imported-data integrity and the six deterministic reproduction checks. See REPRODUCE.md for the scope of other commands and the two large regenerable Floquet cohort files omitted from Git.

The immutable evidence imports retain original notes, code and protocols. Root documents define the current scope; historical instructions inside evidence are not new assignments. The legacy tree must remain unchanged. Do not alter Subset-states or the other frozen repositories as part of this project. External submission or contacting collaborators requires authorization for that action.
