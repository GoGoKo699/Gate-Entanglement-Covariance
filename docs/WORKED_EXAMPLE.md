...
Every same-order covariance weight is nonnegative, and $`c_{\alpha,3}=-2\alpha(\alpha-2)/[(\alpha+2)(\alpha+3)]`$ is nonzero at every fixed positive order except two. Thus the covariance difference is strict at all those orders, including the continuous order-one case. The [saved analytical evaluations](../studies/haar/numerics/operator_results/equal_purity_analytic.json) evaluate this witness. The comparison fixes operator purity, not Hamiltonian strength or duration. It does not infer the full gate from a single extra entropy measurement.
