...
Its typical size is order one for the fixed test functions used here. A central limit theorem for these traces concerns spectral fluctuations around the mean, not the trace itself, which has size of order $`d`$.
...
Use the book's explicit exact-mean centering on folios 156–157 when reading fluctuation limits. The selected route excludes the uncentered convergence wording in Theorem 1 on folio 128 and the inconsistent orthogonality formula in Exercise 12(iii), Eq. (5.32). Neither is needed for our argument: $`X_d`$ above fixes the centering, and the displayed definition of $`C_k`$ fixes the amplitudes. The [source caveats](TUTORIAL_SOURCE.md#source-caveats) record these discrepancies and their scope.
...
The $`L^2`$ remainder requires more than formal Taylor expansion. The proof uses variance convergence for uniform integrability and bounds the combined normalized entropy on exceptional events by $`0\le S_\alpha\le\log d`$. Subtracting its exact mean at finite $`d`$ preserves the vanishing remainder; replacing that mean by only its leading asymptotic value is unnecessary.
