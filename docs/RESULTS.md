...
The observable below is the rescaled mean-square entropy increment, `d² E[(Sα(Uψ)−Sα(ψ))²]`. Haar stationarity relates it to the covariance theorem. Each `±` gives one sample standard error over the 96 independent inputs at that size.
...
Sources: [protocol](../evidence/checkpoint07/checkpoints/07/PROTOCOL.md), [frozen predictions](../evidence/checkpoint07/checkpoints/07/results/pilot_frozen.json), [implementation](../evidence/checkpoint07/checkpoints/07/numerics/entropy_pilot.py), [complete summary](../evidence/checkpoint07/checkpoints/07/results/pilot_summary.json), and saved arrays at [d=32](../evidence/checkpoint07/checkpoints/07/results/pilot_d32.npz) and [d=64](../evidence/checkpoint07/checkpoints/07/results/pilot_d64.npz). The displayed means and standard errors agree with direct recomputation from these arrays. The [original validation record](../evidence/checkpoint07/VALIDATION.json) documents the historical fresh-directory reproduction.

Earlier ZZ and diagonal-gate cohorts remain separate inherited evidence. They are not pooled into the 192-state pilot. The earlier 128-state diagonal comparison contains substantial adverse deviations: at `d=128`, gate B and time `π/4`, the measured increment statistics were `0.107929 ± 0.016960` against `0.181407` at order `1/2`, and `0.188963 ± 0.027740` against `0.326083` at order `1`. Some paired gate differences also had the opposite sign to their limiting prediction. Sampling and finite-size contributions to these discrepancies remain unresolved. See the [saved diagonal summary](../evidence/checkpoint07/numerics/operator_results/summary.json) and [interpretation](../evidence/checkpoint07/docs/history/CHECKPOINT06_PROJECT_STATUS.md). The exact recurrence control in that cohort worked to about `1.78×10⁻¹⁵` in entropy.

## Algebraic and deterministic checks

The leading Gaussian contraction calculation checked 68,786 networks across ten gates, including unequal `2×3` active factors. Mixed Chebyshev degrees through three agreed within about `4.57×10⁻¹³`; degree four for eight gates agreed within about `3.79×10⁻¹⁰`, after cancellation of larger monomial terms. This tests low-degree contraction identities in floating-point arithmetic. The all-degree result rests on the proof above. See [code](../evidence/checkpoint07/checkpoints/07/numerics/general_gate_wick.py), [saved contractions](../evidence/checkpoint07/checkpoints/07/numerics/general_gate_wick.json), and the [bounded-validation report](../evidence/checkpoint07/checkpoints/07/REPORT.md).
...
| Whole-system purity identity | 14 gates, 336 fourth-Haar-moment permutation traces; largest unnormalized trace difference about `1.14×10⁻¹³` | [Code](../evidence/checkpoint08/finite_purity/check_exact_global_purity.py), [results](../evidence/checkpoint08/finite_purity/exact_global_purity_results.json) |
| Two subsystem assignments, full-system contraction | 14 active-gate/spectator cases; largest mode-correlation difference about `1.66×10⁻¹⁴` | [Code](../evidence/checkpoint08/finite_purity/check_two_cut_modes.py), [results](../evidence/checkpoint08/finite_purity/two_cut_modes_results.json) |
...
| Separate purity-sum and product-input contractions | 15 active gates at dimensions 2, 3, 4 and five spectator sizes; 75 correlations agree within `1.29×10⁻¹²`; entangling-power formulas within `1.11×10⁻¹⁶` | [Code](../evidence/checkpoint08/numerics/two_cut_check.py), [results](../evidence/checkpoint08/results/two_cut_check.json) |
| Exact gate-design pair | Two full two-copy twirling superoperators agree with Haar within `3.25×10⁻¹⁶` in Frobenius norm; limiting order-three correlation difference is exactly `3√5/20000` | [Code](../evidence/checkpoint08/numerics/design_pair.py), [results](../evidence/checkpoint08/results/design_pair.json), [analytical audit](../evidence/checkpoint08/reviews/DESIGN_PAIR_INDEPENDENT_AUDIT.md) |
